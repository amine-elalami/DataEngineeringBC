...
print("d) Le document de la collection 'books' est :")
pprint(document_books)
...
# Python, Java, C++, Scala.
nombre_livres_programmation = books.count_documents({
    "$or": [
...
result.append("e) Informations statistiques par catégorie : {}".format(statistics))
...
pprint(edited_books)
result.append("f) Les livres publiés après 2009 sont : {}".format(edited_books))
...
result.append("g) Les livres publiés après 2009 sont : {}".format(decomposition_auteurs_et_filtres))
...
result.append("h) Les 10 auteurs les plus prolifiques sont : {}".format(auteurs_prolifiques))
...
result.append("i) La distribution du nombre d'auteurs par livre est : {}".format(distribution_auteurs))
...
result.append("j) La distribution du nombre d'auteurs par livre est : {}".format(occurences_auteurs))

write_result_to_file(result, "mongo_db/res.txt")
